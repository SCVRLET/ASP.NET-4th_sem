...
$(document).on("click", "button.button.like", function () {
    ...
    var likes_number_el = $(this).parent("div").parent("div").find(".like-comment-repost-number")[0];

    $.ajax({
        url: '/Users/LikeOrDeleteLike/',
        type: "POST",
        data: { postId: post_id },
        dataType: "json",
        success: function (response) {
            ...
            if (response.like) {
                $(curr_form).append('<button type="submit" class="button like" >' +
                    '<input class="post-id-value" type="text" name="postId" value="' + response.postId + '" style="display: none" />' +
